...
const handler = async (m, { conn, args, usedPrefix }) => {
   const res = await fetch('https://api.github.com/repos/JOSTINofc/TheMystic-Bot-MD');
   ...
   let txt = `_*< INFO - REPOSITORIO />*_\n\n`;
      ...
      txt += `▢ *URL:* ${json?.html_url || 'https://github.com/JOSTINofc/TheMystic-Bot-MD'}\n\n`;
      txt += `${json?.forks_count || '-'} Forks · ${json?.stargazers_count || '-'} Stars · ${json?.open_issues_count || '-'} Issues\n\n`;
      txt += `*[ ℹ️ ] Puedes descargar este repositorio enviando el comando:*\n_${usedPrefix}gitclone ${json?.html_url || 'https://github.com/JOSTINofc/TheMystic-Bot-MD'}_`;
